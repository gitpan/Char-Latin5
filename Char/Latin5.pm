...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Char::Elatin5::uc($char[$i]) ne Char::Elatin5::lc($char[$i]))) {
            $char[$i] = '[' . Char::Elatin5::uc($char[$i]) . Char::Elatin5::lc($char[$i]) . ']';
        }
...
http://search.cpan.org/dist/Char-Char::Latin5/Char::Latin5/JA.pm
...
Definitions in Char/Elatin5.pm.
  ...
  @{Char::Elatin5::eV}             qr{(?:[^\x0C\x0A\x0D])}
