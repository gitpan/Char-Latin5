...
sub Char::Latin5::escape(;$) {
    ...
    while (not /\G \z/oxgc) { # member
        $e_script .= Char::Latin5::escape_token();
    }
...
# subroutines of package Char::Elatin5
    ...
    elsif (/\G \b Char::Latin5::eval       (?= \s* \{ )                /oxgc) { $slash = 'm//'; return 'eval';              }
    elsif (/\G \b Char::Latin5::eval    \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'eval Char::Latin5::escape'; }
...
# subroutines of package Char::Elatin5
        ...
        elsif ($string =~ /\G \b Char::Latin5::eval       (?= \s* \{ )                /oxgc) { $e_string .= 'eval';              $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::Latin5::eval \b                                /oxgc) { $e_string .= 'eval Char::Latin5::escape'; $slash = 'm//'; }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                              1                        2   3                                 4   5
                q<CORE::eval{local $Char::Latin5::re_t=%s; while($Char::Latin5::re_t =~ %s){%s local $^W=0; local $Char::Latin5::re_r=%s; %s$Char::Latin5::re_t="$`$Char::Latin5::re_r$'"; pos($Char::Latin5::re_t)=length "$`$Char::Latin5::re_r"; } return $Char::Latin5::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::Latin5::re_r=CORE::eval $Char::Latin5::re_r; ' x $e_modifier,            #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2                3                                 4   5  6                    7
                q<(%s =~ %s) ? CORE::eval{%s local $^W=0; local $Char::Latin5::re_r=%s; %s"%s$Char::Latin5::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                       1     2   3                                 4   5 6                         7                                                 8
                q<CORE::eval{local $Char::Latin5::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::Latin5::re_r=%s; %s%s="$`$Char::Latin5::re_r$'"; pos(%s)=length "$`$Char::Latin5::re_r"; $Char::Latin5::re_n++} return %s$Char::Latin5::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3                4                                 5   6 7   8
                q<(%s%s%s) ? CORE::eval{%s local $^W=0; local $Char::Latin5::re_r=%s; %s%s="%s$Char::Latin5::re_r$'"; 1 } : undef> :

                #  1 2 3                    4                                 5   6 7   8
                q<(%s%s%s) ? 1 : CORE::eval{%s local $^W=0; local $Char::Latin5::re_r=%s; %s%s="%s$Char::Latin5::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::Latin5::re_r=CORE::eval $Char::Latin5::re_r; ' x $e_modifier,            #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
This module has run-time routines for use Char::Latin5 software automatically, you
do not have to use.
...
=item Chop string
  ...
  character chopped. The Char::Elatin5::chop subroutine is used primary to remove the newline
...
=item Reverse index string
  ...
  This subroutine works just like Char::Elatin5::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Char::Elatin5::fc subroutine for case-insensitive comparisons via Char::Latin5
  software.
...
=item Fold case string
  ...
  New to Char::Latin5 software, this subroutine returns the full Unicode-like casefold of
...
=item Filename expansion (globbing)
  ...
  The Char::Elatin5::glob subroutine grandfathers the use of whitespace to separate multiple
