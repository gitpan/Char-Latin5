...
BEGIN {
    ...
    sub Char::Elatin5::binmode(*;$);
    sub Char::Elatin5::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Elatin5::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Elatin5::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Elatin5::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Elatin5::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Char::Elatin5::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Char::Elatin5::uc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Split string
  ...
  Scans a Latin-5 $string for delimiters that match pattern and splits the Latin-5
  ...
  pattern, so the delimiters may be of any size and need not be the same Latin-5
  $string on every match. If the pattern doesn't match at all, Char::Elatin5::split returns
  the original Latin-5 $string as a single substring. If it matches once, you get
  ...
  If Latin-5 $string is omitted, the function splits the $_ Latin-5 string.
...
=item Chop string
  ...
  Chops off the last character of a Latin-5 string contained in the variable (or
  Latin-5 strings in each element of a @list) and returns the character chopped.
  The Char::Elatin5::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Elatin5::index($string,$substr,$position);
  $pos = Char::Elatin5::index($string,$substr);

  Returns the position of the first occurrence of $substr in Latin-5 $string.
  The start, if specified, specifies the $position to start looking in the Latin-5
  $string. Positions are integer numbers based at 0. If the substring is not found,
  the Char::Elatin5::index function returns -1.

=item Reverse index string

  $pos = Char::Elatin5::rindex($string,$substr,$position);
  $pos = Char::Elatin5::rindex($string,$substr);

  Works just like Char::Elatin5::index except that it returns the position of the last
  occurence of $substr in Latin-5 $string (a reverse index). The function returns
  -1 if not found. $position, if specified, is the rightmost position that may be
  returned, i.e., how far in the Latin-5 string the function can search.
...
=item Lower case string
  ...
  Returns a lowercase version of Latin-5 string (or $_, if omitted). This is the
...
=item Lower case first character of string
  ...
  Returns a version of Latin-5 string (or $_, if omitted) with the first character
...
=item Upper case string
  ...
  Returns an uppercased version of Latin-5 string (or $_, if string is omitted).
...
=item Make character
  ...
  set. For example, Char::Elatin5::chr(65) is "A" in either ASCII or Latin-5, and
  Char::Elatin5::chr(0x82a0) is a Latin-5 HIRAGANA LETTER A. For the reverse of Char::Elatin5::chr,
  use Char::Latin5::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Elatin5::binmode(FILEHANDLE, $disciplines);
  Char::Elatin5::binmode(FILEHANDLE);
  Char::Elatin5::binmode($filehandle, $disciplines);
  Char::Elatin5::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Elatin5::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Elatin5::open(FILEHANDLE, $expr);
  $rc = Char::Elatin5::open(FILEHANDLE);
  $rc = Char::Elatin5::open(my $filehandle, $mode, $expr);
  $rc = Char::Elatin5::open(my $filehandle, $expr);
  $rc = Char::Elatin5::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Elatin5::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Elatin5::open my $fh, ">logfile" or die ...
  ...
  Char::Elatin5::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Elatin5::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Elatin5::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Elatin5::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Elatin5::open(FH, "<:crlf:encoding(Char::Latin5)", $path) or die "can't open $path: $!";
