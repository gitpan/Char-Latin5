...
my $qq_substr  = qr{(?: Latin5::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $function_getc;        # getc    to getc    or Latin5::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Latin5::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Latin5::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Latin5::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Latin5::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Latin5::re_t=%s; while($Latin5::re_t =~ %s){%s local $^W=0; local $Latin5::re_r=%s; %s$Latin5::re_t="$`$Latin5::re_r$'"; pos($Latin5::re_t)=length "$`$Latin5::re_r"; } return $Latin5::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Latin5::re_r=eval $Latin5::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Latin5::re_r=%s; %s"%s$Latin5::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Latin5::re_n=0; while(%s =~ %s){%s local $^W=0; local $Latin5::re_r=%s; %s%s="$`$Latin5::re_r$'"; pos(%s)=length "$`$Latin5::re_r"; $Latin5::re_n++} return %s$Latin5::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Latin5::re_r=%s; %s%s="%s$Latin5::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Latin5::re_r=%s; %s%s="%s$Latin5::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Latin5::re_r=eval $Latin5::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
expand wildcard arguments supplied to programs. Instead, import of Elatin5.pm
works well.
