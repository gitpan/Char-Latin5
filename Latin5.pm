...
######################################################################
#
# Latin5 - Source code filter to Latin-5 script
#
...
my $function_ord;         # ord()   to ord()   or Latin5::ord()
my $function_ord_;        # ord     to ord     or Latin5::ord_
my $function_reverse;     # reverse to reverse or Latin5::reverse
...
sub Latin5::escape_script {
    ...
    if (m/^ use Elatin5(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Elatin5 %s;\n%s", $Elatin5::VERSION, $use_re_eval); # require run-time routines version

        # use Latin5 version qw(ord reverse);
        $function_ord     = 'ord';
        $function_ord_    = 'ord';
        $function_reverse = 'reverse';
        if (s/^ \s* use \s+ Latin5 \s* ([^;]*) ; \s* \n? $//oxms) {
            ...
            if ($list !~ m/\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Latin5::ord'     if m/\A ord \z/oxms;
                    $function_ord_    = 'Latin5::ord_'    if m/\A ord \z/oxms;
                    $function_reverse = 'Latin5::reverse' if m/\A reverse \z/oxms;
                }
            }
        }
    }
...
# functions of package Elatin5
    elsif (m{\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  }oxgc) { $slash = 'm//'; return $1;                    }
    elsif (m{\G \b bytes::substr \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (m{\G \b chop \b          (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Elatin5::chop';         }
    elsif (m{\G \b bytes::index \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'index';               }
    elsif (m{\G \b Latin5::index \b   (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Latin5::index';         }
    elsif (m{\G \b index \b         (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Elatin5::index';        }
    elsif (m{\G \b bytes::rindex \b (?! \s* => )                }oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (m{\G \b Latin5::rindex \b  (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Latin5::rindex';        }
    elsif (m{\G \b rindex \b        (?! \s* => )                }oxgc) { $slash = 'm//'; return 'Elatin5::rindex';       }
    elsif (m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::lc';           }
    elsif (m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::lcfirst';      }
    elsif (m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::uc';           }
    elsif (m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::ucfirst';      }
    elsif (m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::fc';           }
    ...
    elsif (m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::chr';               }
    ...
    elsif (m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $slash = 'm//'; return 'Elatin5::glob';              }
    elsif (m{\G \b lc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin5::lc_';               }
    elsif (m{\G \b lcfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin5::lcfirst_';          }
    elsif (m{\G \b uc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin5::uc_';               }
    elsif (m{\G \b ucfirst \b    (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin5::ucfirst_';          }
    elsif (m{\G \b fc \b         (?! \s* => )                         }oxgc) { $slash = 'm//'; return 'Elatin5::fc_';               }
    ...
    elsif (m{\G \b chr \b           (?! \s* => )                      }oxgc) { $slash = 'm//'; return 'Elatin5::chr_';              }
    elsif (m{\G \b bytes::ord \b    (?! \s* => )                      }oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (m{\G \b ord \b           (?! \s* => )                      }oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (m{\G \b glob \b          (?! \s* => )                      }oxgc) { $slash = 'm//'; return 'Elatin5::glob_';             }
...
# functions of package Elatin5
        ...
        elsif ($string =~ m{\G \b chop \b                                      }oxgc) { $e_string .= 'Elatin5::chop';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::index \b                              }oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b Latin5::index \b                               }oxgc) { $e_string .= 'Latin5::index';    $slash = 'm//'; }
        elsif ($string =~ m{\G \b index \b                                     }oxgc) { $e_string .= 'Elatin5::index';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::rindex \b                             }oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ m{\G \b Latin5::rindex \b                              }oxgc) { $e_string .= 'Latin5::rindex';   $slash = 'm//'; }
        elsif ($string =~ m{\G \b rindex \b                                    }oxgc) { $e_string .= 'Elatin5::rindex';  $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::lc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::uc';      $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) }oxgc) { $e_string .= 'Elatin5::glob';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b lc \b                                              }oxgc) { $e_string .= 'Elatin5::lc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b lcfirst \b                                         }oxgc) { $e_string .= 'Elatin5::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b uc \b                                              }oxgc) { $e_string .= 'Elatin5::uc_';               $slash = 'm//'; }
        elsif ($string =~ m{\G \b ucfirst \b                                         }oxgc) { $e_string .= 'Elatin5::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ m{\G \b fc \b                                              }oxgc) { $e_string .= 'Elatin5::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ m{\G \b chr \b                                             }oxgc) { $e_string .= 'Elatin5::chr_';              $slash = 'm//'; }
        elsif ($string =~ m{\G \b bytes::ord \b                                      }oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ m{\G \b ord \b                                             }oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ m{\G \b glob \b                                            }oxgc) { $e_string .= 'Elatin5::glob_';             $slash = 'm//'; }
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ m/\A [\x00-\xFF] \z/oxms) and (Elatin5::uc($char[$i]) ne Elatin5::fc($char[$i]))) {
...
Latin5 - Source code filter to escape Latin-5

=head1 SYNOPSIS

  use Latin5;
  use Latin5 version;         --- require version
...
=head1 Calling 'Elatin5::ignorecase()' (Latin5.pm provides)

Latin5.pm applies calling 'Elatin5::ignorecase()' instead of /i modifier.
...
=head1 Un-Escaping \ Of \N, \p, \P and \X (Latin5.pm provides)

Latin5.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Latin5.pm and Elatin5.pm provide)
...
=head1 Un-Escaping bytes::* Functions (Latin5.pm provides)

Latin5.pm removes 'bytes::' at head of function name.
...
=head1 Escaping Built-in Standard Module (Elatin5.pm provides)

Elatin5.pm does "BEGIN { unshift @INC, '/Perl/site/lib/Latin5' }" at head.
Store the standard module modified for Latin5 software in this directory to
override built-in standard modules.

=head1 Escaping Standard Module Content (You do)

You need copy built-in standard module to /Perl/site/lib/Latin5 and change
'use utf8;' to 'use Latin5;' in its. You need help yourself for now.
...
Contrasting is Encode module and describing "use Latin5;" on this software,
