...
BEGIN {
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Elatin5::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Elatin5::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Elatin5::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Elatin5::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
#               $pattern .= "(?:$your_char)",   # UNIX style
            ...
            elsif ((my $uc = Elatin5::uc($char)) ne $char) {
                $pattern .= $uc;
            }
            else {
        ...
        my $matchsub = sub { Elatin5::uc($_[0]) =~ m{\A $pattern \z}xms };
...
This module is a run-time routines of the Latin5.pm.
Because the Latin5.pm automatically uses this module, you need not use directly.
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Elatin5::split returns
...
=item Chop string
  ...
  The Elatin5::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Elatin5::index($string,$substr,$position);
  $pos = Elatin5::index($string,$substr);
  ...
  the Elatin5::index function returns -1.

=item Reverse index string

  $pos = Elatin5::rindex($string,$substr,$position);
  $pos = Elatin5::rindex($string,$substr);

  Works just like Elatin5::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Elatin5::chr(65) is "A" in either ASCII or Latin-5, and
  Elatin5::chr(0x82a0) is a Latin-5 HIRAGANA LETTER A. For the reverse of Elatin5::chr,
  use Latin5::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Elatin5::binmode(FILEHANDLE, $disciplines);
  Elatin5::binmode(FILEHANDLE);
  Elatin5::binmode($filehandle, $disciplines);
  Elatin5::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Elatin5::open(FILEHANDLE, $mode, $expr);
  $rc = Elatin5::open(FILEHANDLE, $expr);
  $rc = Elatin5::open(FILEHANDLE);
  $rc = Elatin5::open(my $filehandle, $mode, $expr);
  $rc = Elatin5::open(my $filehandle, $expr);
  $rc = Elatin5::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Elatin5::open($fh, ">logfile")     # $fh is autovivified
  ...
  Elatin5::open my $fh, ">logfile" or die ...
  ...
  Elatin5::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Elatin5::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Elatin5::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Elatin5::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Elatin5::open(FH, "<:crlf:encoding(Latin5)", $path) or die "can't open $path: $!";
