...
#
# Latin-5 fold case lower case without parameter
#
sub Elatin5::fc_() {
    my $s = $_;
    return join '', map {defined($fc{$_}) ? $fc{$_} : $_} ($s =~ m/\G ($q_char) /oxmsg);
...
#               $pattern .= "(?:$your_char)",   # UNIX style
        ...
        my $matchsub = sub { Elatin5::fc($_[0]) =~ m{\A $pattern \z}xms };
...
=item Chop string
  ...
  character chopped. The Elatin5::chop function is used primary to remove the newline
...
=item Reverse index string
  ...
  This function works just like Elatin5::index except that it returns the byte
...
=item Lower case string
  ...
  You can use the Elatin5::fc function for case-insensitive comparisons via Latin5
  software.
...
=item Fold case string
  ...
  New to Latin5 software, this function returns the full Unicode-like casefold of
...
=item Filename expansion (globbing)
  ...
  The Elatin5::glob function grandfathers the use of whitespace to separate multiple
